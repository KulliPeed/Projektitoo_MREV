...
Selleks loodud juhtimislaud võimaldab saada varajase ja ajakohase ülevaate ettevõtetest, millel esinevad makseraskused koos juhatuse muutustega ning jälgida selliste ettevõtete arvu ja jaotust ajas. Püstitus on maksuhalduri vajadusest lähtuvalt, ent võimalik on võtta kasutusele ka ettevõtetes, sidudes kliendibaasiga ja hinnata seeläbi varakult riske kliendi käitumises. Juhatuse vahetus võib olla varajane signaal pöördumatutest makseraskustest ja seetõttu väärtuslik mitmetele osapooltele, nii Maksu- ja Tolliametile, krediidihalduse ettevõtetele, pankrotihalduritele kui ka ettevõtjatele. 
...
4. Viimase päeva seisuga juhatuse liikme vahetusega maksuvõlglaste nimekiri.
5. Viimase 7 päeva lõikes juhatuse muutusega ettevõtete maksuvõla kogusumma võla vanuse gruppides.
...
Kõik paroolid, secret key väärtused ja andmebaasi DSN-id asuvad `.env` või `.env.superset` failides. Päris `.env` ja `.env.superset` faile ei ole GitHubi commit'itud. Repos on olemas ainult näidisfailid, `.env.superset.example` ja  `.env.example`.
...
2. **Laadimine** — Andmed laaditakse muutmata kujul RAW kihti.
3. **Transformatsioon** —  `refresh_stage_incremental.sh` leiab puuduvad RAW snapshotid ja värskendab ainult puuduvate kuupäevade andmed puhastatud kujul STAGE kihis. `refresh_mart_star.sh` ehitab faktitabeli, ühendab MTA ja RIKi andmed ja arvutab vajalikud faktid (nt juhatuse muutuse, võla vanuse grupid jm) ja dimensioonid stage andmete põhjal.
4. **Testimine** — `run_data_quality_checks.py` kirjutab 18 andmekvaliteedi testi erinevate kihtide (raw, stage, mart) andmete kontrollimiseks, mis salvestuvad `quality` skeemi  (sh. quality.data_quality_results tabelisse) ja kuvatakse sellelt välja ka juhtimislaual.
5. **Juhtimislaud** — näitab viimase päeva juhatuse vahetusega maksuvõlgnike nimekirja, juhatuse vahetusega ettevõtete arvu ja maksuvõlga, nende muutust ajas ning maksuvõlga maksuvõla vanusegruppides. Lisaks ka andmekvaliteedi testide tulemusi.
...
- Piisaval hulgal andmekvaliteedi kontrolle on loodud andmebaasi erinevatele kihtidele, et kiirelt tuvastada millises andmevoo etapis probleem esines ning  toodud välja juhtimislaua vaadetel. Andmekvaliteedi tulemused kirjutatakse eraldi quality skeemi.
- RAW import on idempotentne ja logib tegevused admin.raw_import_audit tabelisse.

**Puudused:**
-  Vale-positiivsed andmekvaliteedi testi tulemused - kolm testi toovad hetkel veel igapäevaselt välja vead, kus tegelikult ei ole tegemist vigadega andmetes, vaid andmekvaliteedi testi skriptid vajavad korrigeerimist. Veateade, et MTA andmed ei vasta formaadile toob välja mitteresidentide registrikoodid, mis algavad tähekombinatsiooniga, kuid tegelikult peaks see skriptis olema lubatud formaadiks. STAGE ja MART kihi snapshot kuupäeva kirjete ja maksuvõla summa pariteedi kontrollid annavad vea, sest kasutavad hetkel kontrolliks erineva sisuga kuupäevi (STAGE snaphot kuupäev vs MART faktitabelis MTA andmetes esinev maksuvõla seisu kuupäev). Aja nappusel, kuid ka teisalt teadlikult  juhtimislaua andmekvaliteedi testide toimivuse näitlikustamise eesmärgil jätsime nimetatud testide skriptid täiendamata.
- Skriptide puhastamine ja kommentaaride lisamine jäid pooleli. Kood töötab, kuid loetavust saaks parandada.
- Andmekvaliteedi FAIL/WARN teavitusi me ei jõudnud luua, tekitasime ainult kokkuvõtvad vaated juhtimislauale.

**Mis edasi:**
- Edasiarendusena kõrvaldaks eelpool nimetatud puudused - täiendaks vale-positiivseid kvaliteeditestide skripte, looks automaatsed teavitused emailile ning puhastaks ja parandaks skriptide loetavust.
